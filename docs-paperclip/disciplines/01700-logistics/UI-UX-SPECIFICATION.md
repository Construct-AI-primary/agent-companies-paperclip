...
description: "Complete UI/UX specification for the 01700 Logistics discipline page — integrated logistics management with field data capture, mobile verification, transport tracking, delivery confirmation, and contract compliance. Incorporates org-agnostic patterns derived from F-Stander logistics knowledge."
author: "DomainForge AI — Logistics Domain"
date: "2026-05-03"
version: "1.0"
status: "active"
tags: logistics, ui-ux, specification, field-capture, mobile, transport, delivery, weighbridge, compliance
related_docs:
...
```mermaid
flowchart LR
    SCAN[Scan QR/Barcode] --> LOOKUP[Lookup Transaction]
    LOOKUP --> VALIDATE{Context Validation}
    VALIDATE -->|weight| WEIGHT_CHECK[Weight Check]
    VALIDATE -->|location| GEO_CHECK[Geofence Check]
    VALIDATE -->|time| TIME_CHECK[Time Window Check]
    VALIDATE -->|device| DEVICE_CHECK[Device Auth Check]
    WEIGHT_CHECK -->|pass| WEIGHT_PASS[Weight OK]
    WEIGHT_CHECK -->|fail| WEIGHT_FAIL[Weight Alert]
    GEO_CHECK -->|pass| GEO_PASS[Location OK]
    GEO_CHECK -->|fail| GEO_FAIL[Geofence Alert]
    TIME_CHECK -->|pass| TIME_PASS[Time OK]
    TIME_CHECK -->|fail| TIME_FAIL[Time Alert]
    DEVICE_CHECK -->|pass| DEVICE_PASS[Device OK]
    DEVICE_CHECK -->|fail| DEVICE_FAIL[Device Alert]
    WEIGHT_PASS --> CAPTURE[Capture & Record]
    GEO_PASS --> CAPTURE
    TIME_PASS --> CAPTURE
    DEVICE_PASS --> CAPTURE
    WEIGHT_FAIL --> ESCALATE[Escalate for Review]
    GEO_FAIL --> ESCALATE
    TIME_FAIL --> ESCALATE
    DEVICE_FAIL --> ESCALATE

    classDef scan fill:#e3f2fd,stroke:#1976d2
    classDef check fill:#fff3e0,stroke:#f57c00
    classDef pass fill:#e8f5e8,stroke:#388e3c
    classDef fail fill:#ffebee,stroke:#d32f2f

    class SCAN,LOOKUP scan
    class VALIDATE,WEIGHT_CHECK,GEO_CHECK,TIME_CHECK,DEVICE_CHECK check
    class WEIGHT_PASS,GEO_PASS,TIME_PASS,DEVICE_PASS,CAPTURE pass
    class WEIGHT_FAIL,GEO_FAIL,TIME_FAIL,DEVICE_FAIL,ESCALATE fail
```
...
### 7.1 Field Capture App
- **Offline-First**: Full offline capability with background sync
- **Camera Integration**: QR/barcode scanning, photo capture for evidence
- **GPS/Location**: Automatic geofence verification
- **Push Notifications**: Assignment alerts, approval notifications
- **Battery Optimization**: Low-power mode for extended field use

### 7.2 Weighbridge Integration
- **Hardware Connection**: Bluetooth/serial weighbridge connectivity
- **Auto-Capture**: Automatic weight reading on stabilization
- **Calibration Alerts**: Scheduled calibration reminders
- **Transaction Linking**: Auto-link weighbridge reading to transaction

### 7.3 Context-Aware Validation (FSP-007)
