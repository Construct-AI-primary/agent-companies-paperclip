...
│   ├── 01901-purchase-orders-page.js    # Main procurement page with chatbot integration
│   └── modals/
...
└── chatbots/
    └── 01900-procurement-chatbot.js     # Chatbot with workflow streaming
...
### **Enhanced Chatbot Workflow Streaming (NEW - 2026-01-28)**

**System**: Event-based chatbot dispatcher with sequential agent execution
**Implementation**: `client/src/pages/01900-procurement/components/01901-purchase-orders-page.js`

**Architecture Components**:
1. **Chatbot Dispatcher**: `window.chatbotDispatcher` object with methods:
...
- **Event Listener**: Listens for `chatbotMessage` custom events
- **Message Display**: Adds messages to conversation in real-time
- **Fallback Mechanisms**: Multiple delivery methods (event dispatch, direct access, container query)
- **Visual Timing**: 400ms delay between agents, 500ms before streaming starts

**User Experience Flow**:
1. User clicks "Create Order" in CreateOrderModal
2. Modal closes immediately (no waiting)
3. Procurement Chatbot opens automatically
4. Workflow initiation message displays (order details, 7-agent overview)
5. Sequential agent execution begins with real-time updates
6. Each agent displays activation, processing, and completion messages
7. Final workflow summary displays with completion confirmation
8. Total processing time: ~4.5 seconds for 7 agents

**Performance Metrics Displayed**:
- Agent activation and completion status
- Processing time for each agent (in milliseconds)
- Template compatibility scores
- Requirements extraction counts
- Compliance check results
- Document preparation metrics
- Specialist assignment counts
...
**Technical Implementation Details**:
- **useEffect Hook**: Initializes chatbot dispatcher on component mount
- **Async/Await**: Sequential agent execution with proper timing
- **Random Metrics**: Mock data generation for realistic workflow simulation
- **Error Handling**: Try-catch blocks for robust execution
- **Cleanup**: No removal of window.dispatcher to support multiple components
- **Event Propagation**: Bubbling events for broad compatibility
...
1. Modal closes instantly when "Create" is clicked
2. Chatbot opens and becomes visible
3. Workflow initiation message appears
4. Each agent activates and completes in sequence
5. Processing times are displayed for each agent
6. Final summary displays with completion confirmation
7. All messages are formatted correctly with markdown
8. Event-based communication works reliably

**Browser Compatibility**:
- CustomEvent API (all modern browsers)
- Async/await (ES2017+)
- Promise-based timing (all modern browsers)
- DOM event propagation (all browsers)
