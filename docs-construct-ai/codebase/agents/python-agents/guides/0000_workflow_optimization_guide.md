...
promptsD# 0000_CONSTRUCT_AI_OPTIMIZATION_WORKFLOW_GUIDE.md - Construct AI System Optimization & Workflow Design Guide
...
## Cross-Discipline Workflow Integration

### **Procurement (01900) → Logistics (01700) Integration**

The Procurement workflow integrates with the Logistics framework when orders are signed by both parties. This integration enables seamless handoff of procurement data for international trade, customs clearance, and supply chain management.

#### **Trigger Event**
- **Event**: `order_signed` (both buyer and supplier signatures complete)
- **Condition**: International shipment (supplier_country ≠ delivery_country)
- **Action**: Logistics Orchestrator Agent receives order data and initiates 5-stage logistics workflow
...
|-------|------|-------------|
| `supplier_country` | VARCHAR(2) | ISO 3166-1 alpha-2 country code |
| `delivery_country` | VARCHAR(2) | ISO 3166-1 alpha-2 country code |
| `required_delivery_date` | DATE | Target delivery date |
| `transaction_currency` | VARCHAR(3) | ISO 4217 currency code |
| `hazardous_materials` | BOOLEAN | Flag for hazardous goods |

**Priority 2: Important Fields (Required for International Logistics)**
| Field | Type | Description |
|-------|------|-------------|
| `port_of_loading` | VARCHAR(5) | UN/LOCODE port code |
| `port_of_discharge` | VARCHAR(5) | UN/LOCODE port code |
| `item_weight` | DECIMAL(10,2) | Weight in kg per item |
| `item_volume` | DECIMAL(10,3) | Volume in CBM per item |
| `country_of_origin` | VARCHAR(2) | ISO country code per item |
...
- **Workflow Configuration**: `/docs/workflows/01900_PROCUREMENT_COMPREHENSIVE_WORKFLOW/01900_PROCUREMENT_WORKFLOW_CONFIGURATION.md`
