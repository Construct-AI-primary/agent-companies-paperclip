...
para_section: pages/disciplines/00850_civil-engineering
gigabrain_tags: disciplines, 00850_civil-engineering
documentation
openstinger_context: discipline-knowledge, 00850_civil-engineering-practices
...
**Location**: Desktop App → Projects → Civil Engineering → Workflow Dashboard

#### **13-Card Workflow Integration**
The Civil Engineering Workflow System provides a comprehensive project management interface with 13 interconnected workflow cards across 5 design phases. This system integrates seamlessly with the existing mobile calculation tools and provides end-to-end project coordination.

#### **Workflow Access via Modal**
```javascript
// Integration in existing civil engineering page
import CivilEngineeringWorkflow from '../pages/00850-civil-engineering/00850-index';
import { Modal } from '@your-ui-library'; // Use your existing modal component
...
const CivilEngineeringPage = () => {
  ...
  // Check if user is in agent state (customize based on your agent system)
  const isAgentState = useAgentState(); // Your agent state hook/check
  ...
  return (
    <div className="civil-engineering-container">
      ...
      <div className="tab-navigation">
        <button onClick={() => setActiveTab('foundation')}>🏗️ Foundation</button>
        <button onClick={() => setActiveTab('structural')}>🔧 Structural</button>
        {/* ... existing tabs */}
      </div>

      {/* Agent State Workflow Button - Only visible when in agent state */}
      {isAgentState && (
      ...
      <Modal
        ...
        size="full" // Full screen modal for workflow
      >
      ...
      {activeTab === 'structural' && <StructuralAnalysisPanel />}
    </div>
  );
};
```
...
#### **Workflow Features**
- **13 Interactive Cards**: Complete civil engineering design workflow
- **Agent Coordination**: DevForge AI, QualityForge AI, and Loopy AI integration
- **Real-time Progress**: Live project status and completion tracking
- **Quality Gates**: Automated validation and approval workflows
- **Dependency Management**: Automatic card unlocking based on completion

#### **Agent Integration**
```typescript
// Agent coordination interface
interface WorkflowAgentIntegration {
  devForgeAgents: Agent[];
  qualityForgeAgents: Agent[];
  loopyAgents: Agent[];
  coordinationProtocol: 'real-time' | 'batch' | 'async';
}
...
#### **Mobile-Desktop Sync**
- **Field Data Integration**: Mobile survey and inspection data flows to workflow
- **Progress Updates**: Real-time construction monitoring updates workflow status
- **Quality Control**: Mobile inspection results trigger workflow validations
- **Document Sync**: Generated reports and drawings sync across platforms
